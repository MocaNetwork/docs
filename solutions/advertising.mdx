...
AIR Kit lets publishers issue verified audience credentials and configure programs that check audience eligibility using ZK proofs. Raw source records remain with the issuer. Encrypted credential payloads may be stored in DStorage, while users control decryption and disclosure to partners.
...
async function getVerifiedAudienceSignal() {
  ...
  const ageResult = await airService.verifyCredential({
    ...
    // Use a ZK age check without requesting the user's date of birth.
    // Successful results include a Verifiable Presentation.
  });
...
| Consent withdrawn | Revoke | Reject future checks once revoked status is available; apply withdrawal to downstream processing |
| KYC-backed audience | Revoke + reissue | Re-issue when KYC result updates |

## Data disclosure

For this audience check, request only the necessary claims and use ZK proofs for conditions that do not require disclosing their underlying values. Successful verification returns a Verifiable Presentation with user-approved claims and any required proof. Configure the program to exclude DOB, email, IP address, and browsing history; the table below describes that limited-disclosure example.
...
Credentials can record consent and support age or eligibility checks, but do not establish regulatory compliance on their own. Review the scope of consent, applicable age requirements, opt-out handling, and retention for each publisher and purpose. Your CMP remains responsible for collecting and managing consent. See the [Compliance FAQ](/solutions/compliance-faq) for storage and disclosure considerations.
...
<Columns cols={2}>
  ...
  <Card title="AIR for Fintech & Payments" icon="credit-card" href="/solutions/fintech">
