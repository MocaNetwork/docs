...
<Steps>
  ...
  <Step title="Your backend issues a KYC credential">
    The moment your KYC provider returns a pass result, your backend calls the AIR Kit Direct Issuance. One call. The credential issues silently — no user action needed.
  </Step>
...
No. AIR Kit never receives PII. The Direct Issuance call contains only the credential attributes you define — derived facts, not raw documents.
