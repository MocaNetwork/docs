...
- **Roaming partner verification** — Let partner carriers verify a subscriber's KYC status via ZK proof without requesting subscriber documents
...
graph TD
    ...
    B -->|KYC pass + activation event| C[On-demand issuance]
...
Your activation system already fires an event when a subscriber passes KYC and activates. Add one server-side issuance call to that event handler.
...
async function issueSubscriberCredential({ userEmail, activation }) {
  ...
  return issueEncryptedCredential({
    ...
    schemaId: process.env.SUBSCRIBER_CREDENTIAL_ID,
    credentialSubject: {
...
async function issueLoyaltyTierIfEligible({ userEmail, tenureMonths }) {
  ...
  await issueEncryptedCredential({
    recipientEmail: userEmail,
    schemaId: process.env.LOYALTY_TIER_CREDENTIAL_ID,
    credentialSubject: {
      carrierId: process.env.CARRIER_ID,
      tier,
      tenureMonths,
...
The roaming partner integrates AIR Kit as a verifier and selects a Verification Program. A successful SDK result includes a Verifiable Presentation with user-approved claims and any required proof. For this example, request KYC status without subscriber identifiers or documents.
...
|---------|:-------------:|-------------|
| Initial activation | Skip if exists | Issue once at first activation; don't re-issue if user reinstalls |
| Plan upgrade / re-KYC | Revoke + reissue | Always reissue with updated `kycLevel` or `planType` |
| Loyalty tier update | Revoke + reissue | Monthly billing cycle refresh |
| Device credential | Skip if exists | One credential per device — immutable proof of ownership |

## Privacy Guarantee

For this KYC-status check, configure the program to use a ZK proof without requesting MSISDN, IMSI, or subscriber documents. Successful verification returns a Verifiable Presentation with any approved claims and required proof; other programs may request additional disclosure.
