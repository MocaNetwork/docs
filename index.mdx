---
title: "Moca Network Developer Docs"
description: "Build with AIR Kit — the identity SDK for portable credentials, ZK-verified compliance, and seamless user onboarding across any platform."
mode: "custom"
---

<div class="hero--wrapper">
  <img class="hero--image" src="/images/landing/hero.png" />
  <div class="hero--content">
    <div
      class="hero--chip"
    >
      AIR Kit SDK · Moca Chain · Identity Infrastructure
    </div>

    <h1 class="hero--title">
      <span class="hero--title-accent">The Identity Layer<br /></span> for the Open Internet
    </h1>

    <p class="hero--subtitle">
      Issue portable credentials, verify users without storing PII, and plug into
      700M+ addressable users — all with a few lines of code.
    </p>

    <div class="hero--cta-row">
      <a href="/airkit/usage/installation" class="hero--cta-primary">
        Quick Setup →
      </a>
      <a href="/airkit/guides/overview" class="hero--cta-secondary">
        View Architecture
      </a>
    </div>
  </div>
</div>


<div class="page--content">

{/* ─── GET STARTED — ROLE-BASED ROUTING ─────────────────────────────── */}
<div class="section--block">
  <p class="section--heading">
    Get started
  </p>

<div class="get-started-row">
<CardGroup cols={2}>
  <Card title="I'm a Partner or Business" icon="building-memo" href="/solutions/index">
    Explore AIR Kit solutions by vertical — loyalty, fintech, gaming, telco, and more. Understand partner economics and compliance.
  </Card>
  <Card title="I'm a developer" icon="rectangle-terminal" href="/airkit/usage/getting-started">
    Install the SDK, authenticate users, and issue your first credential in under 30 minutes.
  </Card>
  <Card title="I'm building on Moca Chain" icon="link-simple" href="/mocachain/index">
    Connect to the L1, run a node, deploy contracts, and explore EVM-compatible tooling on Moca Chain.
  </Card>
  <Card title="I want to understand Moca Network" icon="lightbulb" href="/learn/index">
    Learn about the vision, architecture, and why portable identity matters for the open internet.
  </Card>
</CardGroup>
</div>
</div>

<div class="section--block-sm landing-highlight">
  <p class="landing-highlight--heading">
    On-chain IDV / KYC
  </p>
  <div class="get-started-row">
    <CardGroup cols={2}>
      <Card title="Verify once, reuse across partners" icon="file-contract" href="/kyc/index">
        One **Veriff powered by zkMe** check issues an **AIR Credential** on **Moca Chain**.
        Every other verifier trusts that outcome — no repeat IDV/KYC and no web of
        **pairwise DPAs** just to recognise the same user.
      </Card>
      <Card title="Each verifier, its own disclosure" icon="sliders" href="/kyc/data-collection-and-sharing">
        Per **verification program**: pass/fail only, **selective attributes**, or
        **full raw KYC** under **CAK** and explicit consent — one credential, tuned to
        each product compliance need.
      </Card>
    </CardGroup>
  </div>
</div>

{/* ─── USE CASE CARDS — 3 COL ────────────────────────────────────────── */}
<div class="section--block-sm">
  <p class="section--heading">
    Start with your use case
  </p>

<CardGroup cols={3}>
  <Card title="On-chain IDV / KYC" icon="id-card" href="/kyc/index">
    Veriff powered by zkMe issues a reusable AIR Credential on Moca Chain — verify once, reuse across every partner without pairwise DPAs.
  </Card>
  <Card title="Loyalty programs" icon="gift" href="/airkit/guides/air-for-loyalty">
    Trigger verifiable loyalty credentials from your backend. Let users carry tier status across brands.
  </Card>
  <Card title="Gaming" icon="gamepad-modern" href="/airkit/guides/air-for-gaming">
    Issue achievement credentials and portable player identities. Cross-game SSO with a single AIR Account.
  </Card>
  <Card title="Live events & ticketing" icon="ticket-perforated" href="/airkit/guides/air-for-ticketing">
    Issue attendance credentials at check-in. Gate VIP presales and loyalty tiers to verified fans.
  </Card>
  <Card title="API Reference" icon="diamonds-4" href="/api-reference/introduction">
    Server-to-server REST API for credential issuance, status tracking, and partner operations.
  </Card>
  <Card title="Architecture overview" icon="diagram-project" href="/airkit/guides/overview">
    Data flow, service independence, and technical reference for decision-makers.
  </Card>
</CardGroup>
</div>

{/* ─── FEATURE STRIP ────────────────────────────────────────────────── */}
<div class="feature-strip">
  <p class="feature-strip--heading">
    Why <span class="hero--title-accent">AIR Kit</span>
  </p>

  <div class="airkit--footer">
    <Columns cols={3}>
      <Card img="/images/landing/cred.png">
        One credential, any verifier in the ecosystem. No re-KYC, no re-registration.
      </Card>
      <Card img="/images/landing/lightning.png">
        Issue credentials silently from your backend — no user action required.
      </Card>
      <Card img="/images/landing/gas.png">
        Paymaster covers all on-chain costs — users never touch gas tokens.
      </Card>
    </Columns>
  </div>
</div>

{/* ─── FOOTER LINKS ─────────────────────────────────────────────────── */}
<div class="footer-links">
  <a href="https://developers.sandbox.air3.com/dashboard" class="footer-links__link">
    Dashboard ↗
  </a>
  <a href="https://github.com/MocaNetwork" class="footer-links__link">
    GitHub ↗
  </a>
  <a href="https://discord.gg/mocaversenft" class="footer-links__link">
    Discord ↗
  </a>
  <a href="https://twitter.com/Moca_Network" class="footer-links__link">
    X / Twitter ↗
  </a>
</div>

</div>
