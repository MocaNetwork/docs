...
function getPartnerJwt(recipientEmail) {
  ...
  return jwt.sign(
    // email MUST be the individual recipient's AIR Account email — never reuse across recipients
    { partnerId: process.env.PARTNER_ID, scope: "issue", email: recipientEmail, iat: now, exp: now + 300 },
    privateKey,
...
- [Issuance API Reference](/airkit/usage/credential/issue-on-behalf-api) for endpoint details
