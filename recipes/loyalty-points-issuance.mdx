...
<Warning>
The Partner JWT's signed `email` claim is the routing key that determines which
AIR Account the credential lands in. Resolve it from the triggering event, and
never reuse a partner, service, admin, or static email across recipients.
</Warning>
...
const AIR_API_URL = process.env.AIR_API_URL || "https://air.api.sandbox.air3.com/v2";
const MOCA_CHAIN_API_URL = process.env.MOCA_CHAIN_API_URL || "https://api.sandbox.mocachain.org/v1";
...
async function issueLoyaltyCredential(recipientEmail, tierData) {
  ...
  // 2. Build, sign, and encrypt the credential to the holder
