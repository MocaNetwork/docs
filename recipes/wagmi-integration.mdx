...
description: "Use the @mocanetwork/airkit-connector wagmi connector in React to access AIR Kit smart accounts and sign transactions with useAccount and useConnect hooks."
---
...
Once connected, you can access the underlying `AirService` for credential operations. Verification takes a Partner JWT signed on your server with `scope: "verify"` and the verification program ID:

```ts
const service = airWallet.airService;

const { token } = await fetch("/api/verification-token", { method: "POST" }).then((r) => r.json());
...
if (result.status === "COMPLIANT") {
  // unlock the gated feature
}
```

See [Verify a credential in your app](/recipes/verify-credential-in-app) for the server endpoint and result handling.
