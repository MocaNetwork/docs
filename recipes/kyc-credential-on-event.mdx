...
description: "Wire a KYC-complete webhook to the AIR Kit Issue on Behalf API so verified credentials appear in user AIR Accounts automatically — no active session needed."
---
...
```js
async function waitForOnchain(recipientEmail, coreClaimHash, maxAttempts = 10) {
