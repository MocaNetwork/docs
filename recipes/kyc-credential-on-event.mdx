...
2. A Partner JWT containing `partnerId`, the recipient's `email`, and normal issuer and timing claims.
...
Generate a short-lived Partner JWT containing the recipient's `email`. AIR uses
the signed claim to identify the account to resolve through `initialize-user`.

<Warning>
The signed `email` claim is the routing key that determines which AIR Account
the credential lands in. Resolve it from the triggering event, and never reuse
a partner, service, admin, or static email across recipients.
</Warning>
...
function getPartnerJwt(recipientEmail) {
  const now = Math.floor(Date.now() / 1000);
  return jwt.sign(
    {
      partnerId: process.env.PARTNER_ID,
      email: recipientEmail,
      iss: process.env.ISSUER_ORIGIN,
      iat: now,
      exp: now + 5 * 60,
    },
    privateKey,
...
const AIR_API_URL =
  process.env.NODE_ENV === "production"
    ? "https://air.api.air3.com/v2"
    : "https://air.api.sandbox.air3.com/v2";
const MOCA_CHAIN_API_URL =
  process.env.NODE_ENV === "production"
...
async function issueKycCredential(recipientEmail, kycData) {
  const token = getPartnerJwt(recipientEmail);

  // 1. Resolve or create the recipient's AIR Account
  const initRes = await fetch(`${AIR_API_URL}/auth/initialize-user`, {
    method: "POST",
  ...
  const signed = signVc(vc); // BJJ_SIG_2021, issuer-controlled signing key
  const encrypted = encryptToHolder(signed, publicKey);

  // 3. Store the encrypted envelope in DStorage
  const storeRes = await fetch(`${MOCA_CHAIN_API_URL}/dstorage/vcs`, {
    method: "POST",
