...
description: "Wire a KYC-complete webhook to AIR credential issuance so verified credentials appear in user AIR Accounts automatically — signed by the issuer, encrypted to the holder, stored in DStorage, no active session needed."
---

This recipe shows how to issue a verifiable KYC credential the moment your identity provider confirms a user, using [Issue on Behalf](/airkit/usage/credential/issue-on-behalf) script-based direct issuance. The user never needs to be in an active session.
  ...
  return jwt.sign(
    {
      ...
      scope: "issue",
      // MUST be the individual recipient's AIR Account email — never reuse across recipients
...
async function issueKycCredential(recipientEmail, kycData) {
  ...
  const envelope = encryptToHolder(signed, publicKey);
...
- [Issuance API Reference](/airkit/usage/credential/issue-on-behalf-api) for full endpoint details
