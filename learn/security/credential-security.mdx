...
description: "How Moca Network credentials are signed by issuers, encrypted to the holder, stored as ciphertext, and verified with program-defined proofs and user-controlled disclosure."
---
...
- The holder decrypts the matching credential and approves the requested disclosure. The SDK generates a ZK proof if the program requires one.
...
Only user-approved claims are disclosed. A ZK proof can establish a condition without revealing its underlying value; disclosed claims may still be personal data. Verification completes off-chain by default; programs that require it can record the proof on-chain through the Universal Verifier.
...
| Moca Chain (on-chain) | Storage metadata and state records; verification proofs when required by the program | Do not treat public metadata as encrypted payloads |
