...
description: "Configure Google Sign-In for AIR Kit on Android and iOS with Firebase or Google Cloud."
icon: "log-in"
---
...
Partner configuration supplies the Google OAuth client behavior expected by AIR Kit. Ensure the **same OAuth clients** you configure in Firebase match what you register in the dashboard.
...
In the Developer Dashboard, configure the Google OAuth client IDs and enable Google as a login method for your partner. If the option does not appear in the hosted login UI, confirm partner settings and environment (sandbox vs production).
