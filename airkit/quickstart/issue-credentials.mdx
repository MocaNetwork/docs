...
<Info>
  ...
  [Direct Issuance and Pre-issue](/airkit/guides/overview#direct-issuance-and-pre-issue-flow).
</Info>
...
- A supported Node.js version for your chosen issuer backend and web framework
...
- Issuer signing keys and an issuer DID registered in the target environment
- A registered public HTTPS JWKS endpoint and a server endpoint to sign Partner JWTs
- A web app whose origin is registered in the Dashboard

**A database is optional.** Credential data can come from an existing API or
service, or from static sample data during development. Add database persistence
if your issuer needs durable issuance history, revocation records, or other
application state. Encrypted credentials are stored in dStorage.

The backend and JWKS endpoint can share a host. See [Issuer backend hosting](/airkit/usage/backend-hosting)
for deployment options and optional PostgreSQL setup.
...
- **Register the issuer.** Use the Dashboard for sandbox setup. Mainnet
  activation requires coordination with the AIR team.
...
The backend examples below use the AIR issuer service. Keep its environment,
commands, and schema implementation together.
...
<CardGroup cols={2}>
  ...
  >
    Issuer endpoints, schema logic, signing, encryption, and dStorage integration.
  </Card>
</CardGroup>
...
5. The SDK requests issuance through AIR, which calls the issuer's `POST /issue-vc`
   with the resolved holder identity, public key, schema, and partner identifier.
6. The issuer authorizes the request, loads authoritative claims, creates and
   signs the VC, encrypts it, and stores it in dStorage.
...
See [Architecture and data flow](/airkit/guides/overview) for the sequence diagram.
...
Clone your fork of [AIR issuer service](https://github.com/MocaNetwork/air-issuer-service),
then install its dependencies:

```bash
git clone https://github.com/MocaNetwork/air-issuer-service.git
cd air-issuer-service
pnpm install
```

Use your fork's URL if you already have one. The backend examples here follow
[revision a084ac1](https://github.com/MocaNetwork/air-issuer-service/tree/a084ac180701858c567f5e01488d558d5b3d4f18);
check your chosen revision's README and environment template when upgrading.
...
```bash
# Optional: PostgreSQL for persistent issuer records
...
and `kid`. Generate and publish the document using [JWKS endpoint setup](/airkit/usage/jwks-setup).
The referenced service reads this value at startup.
...
Copy the BJJ issuer DID from the `Issuer DID:` startup log. It is derived from
`SEED` and the configured identity network. You can also retrieve it using the
service's REPL command documented in its README. Register the DID used to sign
your BJJ credentials.

<Accordion title="Optional: persist issuer records in PostgreSQL">
  Configure `DATABASE_URL`, then apply migrations before starting the service:

  ```bash
  npx mikro-orm migration:up
  ```

  Database-backed history and revocation records depend on this persistence.
  When running without a database, skip migrations and do not rely on database
  history or durable revocation records. See [Issuer backend hosting](/airkit/usage/backend-hosting).
</Accordion>

Check startup logs and the registered issuer endpoints when testing deployment.
...
Expose the issuer backend over public HTTPS. In sandbox, register the issuer
under **Dashboard → Issuer → Settings**, then configure its endpoint URLs:

| Item       | Value                                        |
| ---------- | -------------------------------------------- |
| Issuer DID | The BJJ issuer DID from the backend startup log |
| API key    | The backend `API_KEY`                        |
| Partner ID | Your Dashboard Partner ID                    |
| Available credentials URL | `https://issuer.example.com/available-vc` |
| Issuance URL | `https://issuer.example.com/issue-vc` |
...
For mainnet, provide the required issuer configuration to the AIR team for
activation. See [Mainnet setup](/airkit/usage/credential/issuing-credentials#mainnet-setup).

After registration:

1. Confirm the Issuer DID in the Dashboard matches
   the backend's BJJ issuer DID.
...
7. Returns HTTP `201` with an empty response body in the referenced issuer service.
...
```bash
pnpm add @mocanetwork/airkit@1.11.0
```
...
For the `BJJ_SIG_2021` backend issuance flow shown here, pass `{}`: the AIR
credential UI fetches encrypted previews through AIR and the issuer backend
supplies the authoritative claims. This example does not apply to legacy
`IDEN3_MTP` programs, which use a different flow.

The SDK promise resolves on success and rejects on failure
It does not return the VC or a dStorage path. Handle the SDK result separately from backend storage records.
...
3. Confirm the issuance program uses the same schema registered in the issuer
   backend.
4. Confirm AIR has both issuer endpoint URLs and the matching `API_KEY`.
...
and your HTTPS domain or tunnel is reachable by AIR. Update `ISSUER_ORIGIN` and
the registered URLs when a development tunnel changes.
