...
if (result.status === "Compliant") {
  // Verification successful - result includes a W3C Verifiable Presentation
  ...
  if (credential && typeof credential !== "string") {
    console.log("Disclosed claims:", credential.credentialSubject);
  }

  // proof is only present when the program requires a zero-knowledge proof
  const proof = Array.isArray(vp?.proof) ? vp.proof[0] : vp?.proof;
  if (proof) {
    console.log("Transaction hash:", proof.transactionHash); // on-chain programs only
  }
