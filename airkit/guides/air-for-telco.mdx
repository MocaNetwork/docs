...
const AIR_API_URL = 'https://air.api.sandbox.air3.com/v2';
const MOCA_CHAIN_API_URL = 'https://api.sandbox.mocachain.org/v1';
...
async function issueEncryptedCredential({ recipientEmail, schemaId, credentialSubject }) {
  ...
  const init = await fetch(`${AIR_API_URL}/auth/initialize-user`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-partner-id': process.env.PARTNER_ID },
    body: JSON.stringify({ partnerJwt: token }),
  }).then((r) => r.json());
  ...
  const res = await fetch(`${MOCA_CHAIN_API_URL}/dstorage/vcs`, {
    method: 'POST',
