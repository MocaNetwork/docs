...
graph TD
    ...
    B -->|"Scan detects fan's AIR Account email"| C[Direct Issuance]
...
Your venue scanner calls your backend on each valid scan. The backend issues the credential server-side — the fan does nothing, no wallet open, no UX friction at the door.
...
async function issueAttendanceCredential({ fanEmail, eventDetails }) {
  ...
  const envelope = encryptToHolder(signVc(vc), init.publicKey);
...
// issueLoyaltyTier uses the same Direct Issuance pattern — see AIR for Loyalty guide
```
...
<Steps>
  ...
  <Step title="Your backend issues the attendance credential">
    Direct Issuance fires silently. The fan's AIR Account receives the credential on-chain.
  </Step>
