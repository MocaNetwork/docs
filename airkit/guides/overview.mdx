...
Use this path when a **backend event** should trigger issuance without the user being present. Your issuer backend resolves the holder, signs and encrypts the credential, and stores it through DStorage — the end user does not interact with AIR.
...
sequenceDiagram
    ...
    Server->>Server: Sign Partner JWT { partnerId, email, ... }
    Server->>API: POST /auth/initialize-user { partnerJwt }
    API-->>Server: { did, publicKey }
    Server->>Server: Build, sign, and encrypt credential
