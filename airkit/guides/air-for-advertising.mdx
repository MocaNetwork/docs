...
graph TD
    ...
    B -->|Direct Issuance| C[AIR Kit / Moca Chain]
...
async function issueEncryptedCredential({ recipientEmail, schemaId, credentialSubject }) {
  ...
  const envelope = encryptToHolder(signVc(vc), init.publicKey); // BJJ_SIG_2021, encrypted to holder

  const res = await fetch(`${BASE_URL}/dstorage/vcs`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-partner-auth': token },
    body: JSON.stringify({ holderDid: init.did, schemaId, envelope, metadata: { programId: schemaId } }),
  });
...
<Columns cols={2}>
  <Card title="Direct Issuance — Concepts" icon="book" href="/airkit/usage/credential/direct-issuance">
    Server-side issuance without user presence.
  </Card>
  <Card title="Direct Issuance — API" icon="code" href="/airkit/usage/credential/issuance-api">
