...
- Offchain credential issuance and verification options (`offchain`, `waitForOnchainCompletion`, `requireZkp`) — see [Issuing credentials](/airkit/usage/credential/issuing-credentials) and [Verifying credentials](/airkit/usage/credential/verify)
...
- Issue-on-behalf accept/reject flows for new and existing users — see [Issue on Behalf](/airkit/usage/credential/issue-on-behalf)
