...
description: "Self-service Developer Dashboard for AIR Kit partners — manage your partner account, configure issuer programs, and set up verifiers in one place."
---
...
| **JWKS URL**     | **Blocking** for SDK `issueCredential` and `verifyCredential` as well as on-demand issuance. Must be a public HTTPS URL reachable from AIR servers. Also required if you use the [bring your own auth](/airkit/usage/user-authentication#custom-auth-bring-your-own-auth) feature. See [JWKS endpoint setup](/airkit/usage/jwks-setup). |
