...
| **JWKS URL**     | **Blocking** for SDK `issueCredential` and `verifyCredential` as well as Direct Issuance. Must be a public HTTPS URL reachable from AIR servers. Also required if you use the [bring your own auth](/airkit/usage/user-authentication#custom-auth-bring-your-own-auth) feature. See [JWKS endpoint setup](/airkit/usage/jwks-setup). |
