...
description: "Initialize the AIR Kit SDK in Web and Flutter apps — create the AirService instance, configure the environment, and preload wallet and credential modules."
---
...
await airService.init({
  ...
  sessionConfig: {
    locale: "en", // Optional: Enforce a locale; omit for automatic language selection
...
AIR Kit bundles `en`, `ja`, `ko`, `tr`, and `vi`. To customize copy or add languages, use hosted JSON in partner configuration. See [Language & currency configuration](/airkit/usage/config-language).
