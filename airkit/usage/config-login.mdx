...
type LoginMethod =
  ...
  | "passwordlessPhone" // Phone-based passwordless login
  | "passwordlessPhoneToggle" // Phone input revealed behind a link
  | "passwordlessEmailPhone" // One input accepting email or phone
...
- **`passwordlessPhone`**: Phone-number passwordless authentication via SMS one-time password
- **`passwordlessPhoneToggle`**: Same as `passwordlessPhone`, with the input collapsed behind a link
- **`passwordlessEmailPhone`**: A single input that accepts either an email address or a phone number, letting the user choose without an explicit switch
- **`google`**: Google OAuth login integration
- **`wallet`**: Wallet-based authentication
- **`passkey`**: Passkey login

<Info>
  Phone number login (`passwordlessPhone`, `passwordlessPhoneToggle`, `passwordlessEmailPhone`) requires AIR Kit `1.11.0` or later.
</Info>
...
// Phone-first, with Google as an alternative
loginMethods: ["passwordlessPhone", "google"];

// Let the user enter either an email or a phone number
loginMethods: ["passwordlessEmailPhone", "google", "wallet"];
```
