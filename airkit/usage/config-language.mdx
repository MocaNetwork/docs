...
AIR Kit includes English (`en`), Japanese (`ja`), Korean (`ko`), Turkish (`tr`), and Vietnamese (`vi`). English is always the fallback for missing translations. You can override copy or add languages with hosted JSON files registered in partner configuration. The SDK selects the language; it does not accept translation strings.
...
| `localeUrls` | Locale code → JSON URL for all AIR Kit interfaces, where available. Otherwise, register the same combined JSON URL in all four maps below. |
| `authLocaleUrls` | Locale code → JSON URL for authentication. |
| `walletLocaleUrls` | Locale code → JSON URL for the wallet. |
| `recoveryLocaleUrls` | Locale code → JSON URL for recovery. |
| `credentialLocaleUrls` | Locale code → JSON URL for credentials. |

## Customize copy or add a language

1. Use the English catalog's key structure. A combined file covers all interfaces; an interface-specific file must match that interface's composed catalog.
...
3. Register the URLs in partner configuration. If you use a non-empty `supportedLocales` list, include the new locale.
...
{
  "defaultLocale": "es",
  "authLocaleUrls": { "es": "https://cdn.example.com/airkit/i18n/es.json" },
  "walletLocaleUrls": { "es": "https://cdn.example.com/airkit/i18n/es.json" },
  "recoveryLocaleUrls": { "es": "https://cdn.example.com/airkit/i18n/es.json" },
  "credentialLocaleUrls": { "es": "https://cdn.example.com/airkit/i18n/es.json" }
}
```

<Note>
English overrides customize the fallback copy. Bundled translations take precedence over that copy, so also provide a locale-specific override to customize wording for Japanese, Korean, Turkish, or Vietnamese users. Interface-specific overrides take precedence over shared overrides for the same locale.
</Note>
