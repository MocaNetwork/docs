...
description: "Set up the public HTTPS JWKS endpoint AIR Kit requires for issueCredential, verifyCredential, and server-side issuance, and register it in the Dashboard."
keywords: ["jwks", "partner jwt", "issueCredential", "verifyCredential", "kid", "server-side issuance", "blocking", "prerequisite", "well-known"]
---
...
| Server-side issuance (REST) | Server-to-server | **Yes** |
...
There is no single canonical JWKS URL — you must register the one your deployment actually serves. Different starting points expose it at different paths.
...
| Your own implementation | Whatever route you define | Common choices are `/.well-known/jwks.json` or `/api/jwks` |
