...
description: "Verifier guide for AIR Kit Compliance Access Key (CAK) — configure verification programs, capture user consent, receive private keys, and decrypt user data."
---
...
if (result.status === "Compliant") {
  const { cakPrivateKey, verifiablePresentation } = result;
...
| `verifiablePresentation` | `object` | W3C Verifiable Presentation with the disclosed claims and any proof material. See [Verifying credentials](/airkit/usage/credential/verify#response) |
