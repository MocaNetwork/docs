...
1. A backend event fires — KYC passed, purchase confirmed, check-in scanned — or you run a batch from a CSV.
...
sequenceDiagram
    autonumber
    participant Script as Issuer job / air-issuer-service
...
Direct issuance authenticates via a **Partner JWT** attached to each request, signed with `scope: "issue"`. The JWT does **not** carry an `email` claim — the recipient is identified when you call `initialize-user`. For setup, key generation, and examples see [Partner Authentication](/airkit/usage/partner-authentication).

## Recipient email (passed to initialize-user)

The recipient's email is the routing key that determines which AIR Account the credential lands in — but it is supplied in the `initialize-user` request body, not the JWT. Because it is the routing key:
