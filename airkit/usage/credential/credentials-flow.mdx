...
description: "How AIR Credentials work — issuer, holder, and verifier roles, signature-based proofs, encrypted DStorage, and program-driven verification on Moca Chain."
---
...
- **Selective-disclosure credentials.** New integrations use `SD_JWT_VC`. The issuer signs the credential and the holder discloses only the claims required by a verifier.
- **Issuer sovereignty.** Issuers control their own source data and signing keys through an Issuer Backend. AIR never holds issuer keys, signs on the issuer's behalf, or sees plaintext data.
...
2. Configure the SD-JWT issuer identifier, signing key, and public issuer JWKS.
3. Configure `SD_JWT_VC` as the proof type for new programs.
4. Create or search for a relevant Credential Schema.
5. Create an issuance program.
6. Integrate an Issuer Backend so AIR can call `available-vc` and `issue-vc` after your frontend starts `air.issueCredential` — or use [on-demand issuance](/airkit/usage/credential/issuing-credentials#on-demand-issuance) when a backend event should issue without a user session.
