...
sequenceDiagram
    ...
    CAPI->>Iss: POST /available-vc (holderDID, pubKey, userId, filters)
    ...
    CAPI->>Iss: POST /issue-vc (holderDID, pubKey, userId, schemaId)
    Iss->>DS: Retrieve final user data
    Iss->>Iss: Build VC, sign with issuer key, encrypt to user public key
    Iss->>Chain: POST /dstorage/vcs (fielded encrypted credential payload)
    Chain-->>Iss: 201 { storagePath }
    Iss-->>CAPI: Success (empty response body)
