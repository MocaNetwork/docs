...
description: "Issue verifiable credentials from the AIR Kit SDK — define schemas, configure issuance programs, and manage the credential lifecycle as an Issuer."
---
...
| `waitForOnchainCompletion` | `boolean` | Optional. When `true`, wait for on-chain confirmation before resolving. |
| `legacy` | `boolean` | Optional. Use the legacy issuance path when required for compatibility. |
...
| `issuerDid` | `String` | Your Issuer DID. |
| `credentialId` | `String` | Program ID for the credential being issued. |
| `credentialSubject` | `Map<String, dynamic>` | Object containing the credential's claims and attributes for the subject. |
| `curve` | `String?` | Optional. Elliptic curve for compliance encryption key generation (for example `secp256r1`). |

**Response:**
Returns a `CredentialIssuanceResult` with optional `cakPublicKey` when compliance encryption is enabled. See [Reference](/airkit/usage/reference) (Flutter tab).
