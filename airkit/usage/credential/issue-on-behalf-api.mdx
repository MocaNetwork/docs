...
- Read concepts first: [Issuing Credentials](/airkit/usage/credential/issuing-credentials) and [Issue on Behalf](/airkit/usage/credential/issue-on-behalf)
...
Requests authenticate via a **Partner JWT** in the `x-partner-auth` header. The JWT must include the target user's `email` as a signed claim, along with `partnerId` and `scope`. The header must include `kid` matching a key in your JWKS endpoint and `typ: "JWT"`. See [Partner Authentication](/airkit/usage/partner-authentication).
...
async function storeVc(recipientEmail, holderDid, schemaId, envelope, metadata) {
  const token = await getPartnerJwt(recipientEmail);
...
async function issue(recipientEmail, schemaId, credentialSubject) {
  ...
  const { storagePath } = await storeVc(recipientEmail, did, schemaId, envelope, {
