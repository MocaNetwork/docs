...
Under the hood, the SDK loads the holder's matching encrypted credential from DStorage, decrypts it locally, obtains consent for disclosure, and generates a ZK proof when the program requires one. AIR handles ciphertext and metadata; the verifier receives the user-approved disclosures. The proof is checked against the program's settings — off-chain by default, or recorded on-chain when the program requires it. A successful result returns the compliant payload above, whose `verifiablePresentation` contains the disclosed claims and any proof material.
