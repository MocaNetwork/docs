...
description: "Verify AIR Credentials with zero-knowledge proofs — validate user claims against predefined conditions without ever accessing the underlying sensitive data."
---
...
| `offchain` | boolean | No | Optional. Run verification offchain when supported by your program. |
| `legacy` | boolean | No | Optional. Use the legacy verification path when required for compatibility. |
| `requireZkp` | boolean | No | Optional. When `true` (non-legacy), generate a Groth16 ZKP for the program's circuit family. Defaults to `false`. |
