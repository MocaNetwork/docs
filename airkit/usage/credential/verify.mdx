...
| `verifiablePresentation` | `object` | W3C Verifiable Presentation containing the disclosed claims and proof material. |
| `transactionHash` | `string` | Present when the program uses on-chain verification: the on-chain record of the proof. |
| `cakPrivateKey` | `string` | Present only when compliance encryption is enabled. Used to decrypt compliance data encrypted at issuance. |
