...
description: "Set up your AIR Kit partner account on the Developer Dashboard: get your Partner ID, register JWKS, and configure credential services."
---
...
<Note>
  Credential services require additional Dashboard configuration. For a new
  SD-JWT issuer, deploy the issuer backend first, verify its public issuer
  metadata, then save the issuer identifier, endpoint URLs, and API key under
  **Issuer → Settings**. See [Issuing
  Credentials](/airkit/usage/credential/issuing-credentials).
</Note>
