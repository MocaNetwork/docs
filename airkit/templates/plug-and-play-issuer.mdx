...
description: "Fork, configure, and deploy the AIR Credential Issuer template — a ready-to-use issuer app for publishing verifiable credentials in under 30 minutes."
---
...
The **JWKS URL** allows AIR Kit to verify the JWTs signed by your deployed application. Without a working public HTTPS JWKS URL registered in the dashboard, `issueCredential` and `verifyCredential` calls fail.
...
<Info>
  This template hosts JWKS at `<host>/jwks.json`. The [`air-examples`](https://github.com/MocaNetwork/air-examples) repo uses `/api/.well-known/jwks` instead. **Register the exact URL your app actually serves** — there is no path discovery or fallback. For the full procedure (key generation, `kid` rule, path matrix, local HTTPS tunnels), see [JWKS endpoint setup](/airkit/usage/jwks-setup).
</Info>
