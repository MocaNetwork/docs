...
### Credential not stored (no storagePath)

Issuance stores an encrypted envelope in DStorage and returns a `storagePath`. Issuance typically completes in ~1–4 seconds.
...
- Do not rebuild and re-issue on every transient failure — retry the store step with the same envelope.

### Issuance returns 400

- Verify `holderDid`, `schemaId`, and the encrypted `envelope` are all present and correctly typed.
...
- Confirm issuance returned a `storagePath`.
