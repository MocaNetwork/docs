...
These errors apply to the AIR Kit REST API (e.g. [Direct Issuance](/airkit/usage/credential/issuance-api)).

| HTTP status | Likely cause | How to fix |
| --- | --- | --- |
| 400 | Missing or invalid request fields | Verify `holderDid`, `schemaId`, and the encrypted `envelope` are present and correctly typed, and that the credential data matches your schema. |
...
| 403 | Feature not enabled or schema not allowed | Confirm the feature (e.g. Direct Issuance) is enabled for your partner account in the Developer Dashboard. Check that your `issuerDid` owns the schema. |
