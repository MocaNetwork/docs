...
description: "Selective Disclosure lets verifiers read specific credential fields after a successful zero-knowledge proof — opt-in by the issuer, sandbox only."
---
...
By default, AIR Kit credential verification follows the settings in your Verification Program, which define the requested claims and whether a zero-knowledge proof is required. Successful verification returns a Verifiable Presentation with user-approved claims and any required proof. A proof-only check can establish a condition without revealing its underlying value.

**Selective Disclosure** lets the Issuer opt-in to data disclosure for a credential. Once enabled, a Verifier can request specific fields from the credential subject — or the entire credential subject — with user approval. A zero-knowledge proof accompanies the result when required by the program. The Verifier still validates the credential cryptographically; disclosure is layered on top of that result.
...
if (credential && typeof credential !== "string") {
  console.log(credential.credentialSubject); // your requested fields
}
```
...
<Note>
  **Deprecated as of 1.11.** Disclosure used to be returned on a top-level `disclosedData` object alongside `zkProofs` and `transactionHash`. Those fields are no longer populated — read `verifiablePresentation` instead. See the [1.11.1 release notes](/airkit/release-notes).
</Note>
