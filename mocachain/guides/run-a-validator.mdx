...
description: "Run a Moca Chain validator node — hardware requirements, staking, key management, CometBFT configuration, and steps to join the validator set."
---
...
mocad tx authz grant ${GOV_ADDR} generic --msg-type=/cosmos.staking.v1beta1.MsgDelegate --keyring-backend test --chain-id "moca_222888-1" --from ${DELEGATOR_ADDR} --node "https://tm-rpc.testnet.mocachain.dev:443" -b sync --gas "200000000" --fees "100000000000000000000amoca" --yes
```
...
DELEGATOR_ADDR=$VALIDATOR_ADDR
GOV_ADDR=0x7b5Fe22B5446f7C62Ea27B8BD71CeF94e03f3dF2
mocad tx authz grant ${GOV_ADDR} delegate --allowed-validators=${VALIDATOR_ADDR} --spend-limit=10000000000000000000000amoca  --keyring-backend test --chain-id "moca_222888-1" --from ${DELEGATOR_ADDR} --node "https://tm-rpc.testnet.mocachain.dev:443" -b sync --gas "2000000" --fees "1000000000000000000amoca" --yes
```
...
VALIDATOR_ADDR=$(mocad keys show validator -a --keyring-backend test)
DELEGATOR_ADDR=$VALIDATOR_ADDR
mocad tx gov submit-proposal ./proposal.json --keyring-backend test --chain-id "moca_222888-1" --from ${DELEGATOR_ADDR} --node "https://tm-rpc.testnet.mocachain.dev:443" -b sync --gas "600000" --fees "100000000000000000000amoca" --yes
...
```bash
mocad q tx BB845EA77C5051A8885FCEB6FAF06E279305D9DE7B03DBA8915D1A1E2804B3AF --node "https://tm-rpc.testnet.mocachain.dev:443" --output json | jq '.logs[] | .events[] | select(.type == "submit_proposal") | .attributes[] | select(.key == "proposal_id") | .value | tonumber'
...
```bash
curl -s https://api.testnet.mocachain.dev/cosmos/gov/v1/proposals/3 | jq
...
```bash
mocad query staking validators --node "https://tm-rpc.testnet.mocachain.dev:443"
